...
def get_split_index(x, set):
    return int(x * len(set))

def split_first_x(x, zero_set, one_set, two_set):

    return zero_set[:get_split_index(x, zero_set)] + \
            one_set[:get_split_index(x, one_set)] + \
            two_set[:get_split_index(x, two_set)]

def split_in_range(a, b, zero_set, one_set, two_set):

    return zero_set[get_split_index(a, zero_set):get_split_index(b, zero_set)] \
        + one_set[get_split_index(a, one_set):get_split_index(b, one_set)] \
        + two_set[get_split_index(a, two_set):get_split_index(b, two_set)]

def accuracy(nb_predictions, dt_predictions, rf_predictions, test_y):
    accuracy_count = 0
    for nb, dt, rf, ty in zip(nb_predictions, dt_predictions, rf_predictions, test_y):
        count = 0
        if nb == ty: count += 1
        if dt == ty: count += 1
        if rf == ty: count += 1

        if count >=2: accuracy_count += 1

    return accuracy_count / len(test_y)

if __name__ == '__main__':
    a = float(input())
    b = float(input())

    zero_set = [row for row in dataset if row[-1] == 0]
    one_set = [row for row in dataset if row[-1] == 1]
    two_set = [row for row in dataset if row[-1] == 2]

    test_set = zero_set[int(b * len(zero_set)):] + one_set[int(b * len(one_set)):] + two_set[int(b * len(two_set)):]
    ...
    train_cl1 = split_first_x(a, zero_set, one_set, two_set)
    cl1_x = [row[:-1] for row in train_cl1]
    cl1_y = [row[-1] for row in train_cl1]

    cl1.fit(cl1_x, cl1_y)

    train_cl2 = split_in_range(a, b, zero_set, one_set, two_set)
    cl2_x = [row[:-1] for row in train_cl2]
    cl2_y = [row[-1] for row in train_cl2]

    cl2.fit(cl2_x, cl2_y)

    train_cl3 = split_first_x(b, zero_set, one_set, two_set)
    cl3_x = [row[:-1] for row in train_cl3]
    cl3_y = [row[-1] for row in train_cl3]

    cl3.fit(cl3_x, cl3_y)

    nb_predictions = cl1.predict(test_x)
    dt_predictions = cl2.predict(test_x)
    rf_predictions = cl3.predict(test_x)


    print(f'Tochnost: {accuracy(nb_predictions, dt_predictions, rf_predictions, test_y)}')
