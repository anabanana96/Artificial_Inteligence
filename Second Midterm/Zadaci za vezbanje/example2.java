...
def divide_sets(class_zero, class_one, x):
    len_zero, len_one = len(class_zero), len(class_one)
    train_set = class_zero[:int(len_zero * x)] + class_one[:int(len_one * x)]
    test_set = class_zero[int(len_zero * x):] + class_one[int(len_one * x):]

    return train_set, test_set

def get_votes(classifiers, max, test_x):
    votes_zero, votes_one = [], []
    for clf in classifiers:
        p_class = clf.predict([test_x])[0]
        if p_class == 0:
            if clf == max: votes_zero.append(2)
            else: votes_zero.append(1)
        if p_class == 1:
            if clf == max: votes_one.append(2)
            else: votes_one.append(0)

    if sum(votes_zero) > sum(votes_one): return 0
    else: return 1
...
if __name__ == '__main__':
    ...
    train_set, test_set = divide_sets(class_zero, class_one, X)
    ...
    clf3 = RandomForestClassifier(criterion='entropy', random_state=0, n_estimators=4)
    clf4 = MLPClassifier(activation='relu', hidden_layer_sizes=(10), learning_rate_init=0.001, random_state=0)
    ...
    print(f'Najgolema tocnost ima klasifikatorot {classifier_names[max_index]}')

    max_classifier = classifiers[max_index]
    tp_count, fn_count = 0, 0

    for i in range(len(test_x)):
        p_class = get_votes(classifiers, max_classifier, test_x[i])
        true_class = test_y[i]

        if p_class == 1 and true_class == 1: tp_count += 1
        if p_class == 0 and true_class == 1: fn_count += 1

    print(f'Odzivot na kolekcijata so klasifikatori e {tp_count / (tp_count + fn_count)}')
