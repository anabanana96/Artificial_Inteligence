...
if __name__ == '__main__':
    ...
    for row in dataset:
        if row[-1] >= 5:
            new_dataset.append(row[:-1] + [1])
        else:
            new_dataset.append(row[:-1] + [0])

    split_index = int(x * len(new_dataset))

    train_set = new_dataset[split_index:]
    test_set = new_dataset[:split_index]
    ...
    cl1.fit(train_x, train_y)

    importances = list(cl1.feature_importances_)
    least_important_feature = importances.index(min(importances))

    train_x = [[row[i] for i in range(len(row)) if i != least_important_feature] for row in train_x]
    test_x = [[row[i] for i in range(len(row)) if i != least_important_feature] for row in test_x]
    ...
    predictions = cl2.predict(test_x)
    accuracy = calculate(predictions, test_y)
    print(f'Tocnost so StandardScaler: {accuracy}')

    train_x = scaler2.fit_transform(train_x)
    test_x = scaler2.transform(test_x)
    cl3.fit(train_x, train_y)

    predictions = cl3.predict(test_x)
    accuracy = calculate(predictions, test_y)
