...
def calcultate_accuracy(predictions, true_classes):
    correct = 0
...
if __name__ == '__main__':
    ...
    val_x, val_y = split_sets(val_set)
    ...
    accuracy_train = calcultate_accuracy(classifier.predict(train_x), train_y)
    accuracy_val = calcultate_accuracy(classifier.predict(val_x), val_y)

    difference = abs(accuracy_train - accuracy_val)
    relative_difference = difference / accuracy_val

    new_entry = [float(x) for x in input().split(' ')]
    new_entry = scaler.transform([new_entry])

    if (relative_difference >= 0.15):
        train_x = [[row[i] for i in range(len(row)) if i != col_index] for row in train_x]
        val_x = [[row[i] for i in range(len(row)) if i != col_index] for row in val_x]
        new_entry = [new_entry[i] for i in range(len(new_entry)) if i != col_index]
    ...
    predicted_class = classifier.predict(new_entry)[0]
